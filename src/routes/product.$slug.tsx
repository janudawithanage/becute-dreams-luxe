...
function ProductPage() {
  ...
  return (
    <div className="mx-auto max-w-[1400px] px-6 py-16 lg:px-12 lg:py-24">
      <Link to="/shop" className="text-xs uppercase tracking-[0.25em] text-muted-foreground hover:text-foreground">
        ← Back to shop
      </Link>
      ...
      <div className="mt-10 grid grid-cols-1 gap-12 lg:grid-cols-12 lg:gap-16">
        ...
        <div className="lg:col-span-5 lg:pt-8">
          ...
          <h1 className="mt-3 font-display text-5xl leading-[1.02] tracking-tight lg:text-6xl">{product.name}</h1>
          <p className="mt-6 font-display text-3xl">${product.price.toFixed(2)}</p>

          <p className="mt-8 text-base leading-relaxed text-foreground/70 text-pretty">{product.description}</p>

          <div className="mt-10 flex items-center gap-4">
            <div className="flex items-center gap-4 rounded-full border px-5 py-3">
              <button onClick={() => setQty(Math.max(1, qty - 1))} aria-label="Decrease"><Minus className="h-4 w-4" /></button>
              <span className="w-6 text-center text-sm tabular-nums">{qty}</span>
              <button onClick={() => setQty(qty + 1)} aria-label="Increase"><Plus className="h-4 w-4" /></button>
            </div>
            <button
          ...
          <dl className="mt-12 grid grid-cols-2 gap-6 border-t pt-8 text-sm">
            <div><dt className="text-xs uppercase tracking-[0.2em] text-muted-foreground">Material</dt><dd className="mt-1">Premium matte vinyl</dd></div>
            <div><dt className="text-xs uppercase tracking-[0.2em] text-muted-foreground">Finish</dt><dd className="mt-1">Hand-trimmed, soft-touch</dd></div>
            <div><dt className="text-xs uppercase tracking-[0.2em] text-muted-foreground">Edition</dt><dd className="mt-1">Small batch</dd></div>
            <div><dt className="text-xs uppercase tracking-[0.2em] text-muted-foreground">Shipping</dt><dd className="mt-1">Worldwide, soft packed</dd></div>
          </dl>
        </div>
      </div>
      ...
      {related.length > 0 && (
        <div className="mt-32">
          <h2 className="font-display text-4xl tracking-tight lg:text-5xl">You may also <em className="font-light">love.</em></h2>
