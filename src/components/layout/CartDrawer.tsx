        ...
        <>
          ...
          >
            <div className="flex items-center justify-between border-b px-6 py-5">
              <div>
                <p className="text-xs uppercase tracking-[0.25em] text-muted-foreground">Your selection</p>
            ...
            {items.length === 0 ? (
              <div className="flex flex-1 flex-col items-center justify-center px-6 text-center">
                ...
                <p className="mt-2 text-sm text-muted-foreground">Add a little softness to begin.</p>
                <Link
                  to="/shop"
                  onClick={() => setOpen(false)}
              ...
              <>
                  ...
                  {items.map((i) => (
                    <div key={i.product.id} className="flex gap-4">
                      ...
                      <div className="flex flex-1 flex-col">
                        ...
                        <p className="text-xs uppercase tracking-wider text-muted-foreground">${i.product.price} each</p>
                        <div className="mt-auto flex items-center justify-between pt-3">
                          <div className="flex items-center gap-3 rounded-full border px-3 py-1.5">
                            <button onClick={() => setQty(i.product.id, i.qty - 1)} aria-label="Decrease">
                              <Minus className="h-3 w-3" />
                            </button>
                            <span className="w-4 text-center text-xs">{i.qty}</span>
                            <button onClick={() => setQty(i.product.id, i.qty + 1)} aria-label="Increase">
                              <Plus className="h-3 w-3" />
                            </button>
                          </div>
                          <button onClick={() => remove(i.product.id)} aria-label="Remove" className="text-muted-foreground hover:text-foreground">
                ...
                <div className="space-y-4 border-t px-6 py-6">
                  <div className="flex items-baseline justify-between">
                    <span className="text-xs uppercase tracking-[0.25em] text-muted-foreground">Subtotal</span>
